...
def load_and_process_data():
    ...
    # K-Means (n=3, rs=42) - Harus sama dengan Colab
    kmeans = KMeans(n_clusters=3, random_state=42).fit(X_scaled)
    df['cluster'] = kmeans.labels_

    # PENENTUAN KATEGORI BERDASARKAN RATA-RATA MENINGGAL DUNIA
    # Ini memastikan cluster dengan tingkat fatalitas tertinggi selalu berwarna MERAH
    cluster_means = df.groupby('cluster')['jumlah korban meninggal dunia'].mean().sort_values()
    urutan = cluster_means.index.tolist()
    
    # urutan[0] = terendah, urutan[1] = sedang, urutan[2] = tertinggi
...
try:
    ...
    <div style="display: flex; justify-content: center; gap: 20px; font-weight: bold;">
        <span style="color:#D32F2F">&#9679; Rawan Tinggi (Merah)</span>
        <span style="color:#FBC02D">&#9679; Rawan Sedang (Kuning)</span>
        <span style="color:#388E3C">&#9679; Rawan Rendah (Hijau)</span>
    </div>
    ''', unsafe_allow_html=True)
